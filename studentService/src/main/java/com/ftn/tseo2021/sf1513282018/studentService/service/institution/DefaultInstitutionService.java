...
//	No need to explicitly secure methods below since they completely depend on secured methods of another services
	...
	@Override
	public Page<InstitutionTeacherDTO> getInstitutionTeachers(int institutionId, Pageable pageable, TeacherFilterOptions filterOptions) {
		if(!institutionRepo.existsById(institutionId)) throw new ResourceNotFoundException();

		Page<InstitutionTeacherDTO> teachers = teacherService.filterTeachers(institutionId, pageable, filterOptions);
	...
	@Override
	public Page<InstitutionCourseDTO> getInstitutionCourses(int institutionId, Pageable pageable, CourseFilterOptions filterOptions) {
		if(!institutionRepo.existsById(institutionId)) throw new ResourceNotFoundException();

		Page<InstitutionCourseDTO> courses = courseService.filterCourses(institutionId, pageable, filterOptions);
